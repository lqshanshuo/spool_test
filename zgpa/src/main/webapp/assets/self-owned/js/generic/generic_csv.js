var download = function (columnNames,columnData,type) {
  type = type || ".csv";
  var csvData = [];
  csvData.push(columnNames);
  csvData = csvData.concat(columnData);
  var csvFileName = "notebook_download_" + (new Date()).getTime() + type;
  var csvGenerator = new CsvGenerator(csvData, csvFileName);
